...
#SBATCH --job-name=MMA_GW_TestContainer_VFL_server_CPU   # job name
#SBATCH --time=01:00:00                         # dd-hh:mm:ss for the job

#SBATCH -e MMA_GW_TestContainer_VFL_server_CPU-err-%j.log
#SBATCH -o MMA_GW_TestContainer_VFL_server_CPU-out-%j.log
...
# python3 ./grpc/run_vfl_server.py --config ./configs/vfl/server.yaml
cd /scratch/bcbw/parthpatel7173/MMA_GravitationalWave

apptainer exec --nv \
  GW_miniapp_delta.sif \
  python /app/examples/octopus/run_server.py --config /scratch/bcbw/parthpatel7173/MMA_GravitationalWave/examples/configs/server_container.yaml
