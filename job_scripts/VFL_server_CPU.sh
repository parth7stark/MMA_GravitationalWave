...
#SBATCH --job-name=MMA_GW_Test_VFL_server    # job name
#SBATCH --time=01:00:00                         # dd-hh:mm:ss for the job

#SBATCH -e MMA_GW_Test_VFL_server-err-%j.log
#SBATCH -o MMA_GW_Test_VFL_server-out-%j.log
...
conda deactivate
conda deactivate  # just making sure - there shouldnt be any base
module purge
...
# Start the server -- update config before starting

python3 ./examples/octopus/run_server.py --config ./examples/configs/server.yaml
